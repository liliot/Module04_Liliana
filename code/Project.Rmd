...
```{r, warning=FALSE, message=FALSE}
ggplot(Line_clean, aes(x = factor(CalendarYear), y = Licences)) +
...
```{r, warning=FALSE, message=FALSE}
ggplot(Line_clean, aes(x = Licences, y = Days)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(method = "loess", se = TRUE, color = "red") +
  labs(
    title = "Relationship between line fishing licences and annual days at sea",
    x = "no. Licences",
...
The red LOESS curve, along with its confidence band, indicates that as licences increase, fishing effort
(measured by days at sea) generally rises, despite some variation across the dataset.
